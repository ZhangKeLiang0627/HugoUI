...
int16_t list_move_y, list_move_y_trg;
...
uint8_t ListMode2 = false;
uint8_t ListMode1 = true;
...
{   
    ...
    {  
        ...
        for(HugoUIItem_t *item = currentPage->itemHead; (item->lineId <= currentPage->itemMax) && (item->inPage == currentPage->pageId); item = item->next)
        {
            ...
            int16_t Item_y = 12 + currentPage->page_y + item->lineId*13 + currentPage->page_y_forlist;
            switch (item->funcType)
            {   
                // 此页的描述
                case ITEM_PAGE_DESCRIPTION: // 12是调整距离 // 这底下加的统统都是根据情况微调
        ...
        switch (ui_state)
        {
            ...
            case STATE_RUN_PAGE_DOWN: // ui该page向下滚动
                // 判断该往下滚动多少
                if(ListMode2) // 为小屏幕设计的滚动顺序 // 也可以说是ListMode2
                {   
                ...
                else // 通用滚动顺序 // ListMode1
                {
                    ...
                    if(ui_select < SCREEN_HEIGHT/16)
                    {
                        frame_y_trg = ui_select*FONT_HEIGHT;
                    }
            ...
            case STATE_JUMP_PAGE: // 目前发现只显示当前页最流畅 // ui 正在跳转页面
                if(HugoUI_Run(&currentPage->page_y, &currentPage->page_y_trg, 8, 5) == 0)
                {
                    jumpPage_flag |= 0xf0;
                }
    ...
    else
    {
        ...
        for(HugoUIItem_t *item = currentPage->itemHead; (item->lineId <= currentPage->itemMax) && (item->inPage == currentPage->pageId); item = item->next)
        {
            ...
            int16_t Item_x = 48 + currentPage->page_x + icon_move_x*item->lineId;
        ...
        switch (ui_state)
        {
            ...
            case STATE_RUN_PAGE_DOWN: // ui该page向下滚动
                ...
                frame_x -= 32; // frameBox的动效
                if(ui_select == 0) icon_move_x = 0;
            ...
            case STATE_JUMP_PAGE: // 目前发现只显示当前页最流畅 // ui 正在跳转页面
                ...
                if(HugoUI_Run(&currentPage->page_y, &currentPage->page_y_trg, 5, 5) == 0)
                {
                    jumpPage_flag |= 0xf0;
                }
                if(HugoUI_Run(&lastPage->page_y, &lastPage->page_y_trg, 8, 8) == 0)
